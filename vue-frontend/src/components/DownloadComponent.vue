            ...
            <v-row class="justify-center">
              <v-col cols="12" class="d-flex justify-center">
                <v-btn
                  color="primary"
                  @click="downloadVideo"
                  :disabled="!device || (!duration || duration <= 0)"
                >
...
const downloadVideo = async () => {
  try {
    const response = await axios.post(
      `http://${window.location.hostname}:${window.location.port}/api/downloads/`,
      {
    ...
    apiSuccessMessage.value = 'Video download was successful!';
    ...
    if (error.response && error.response.data && error.response.data.detail) {
      ...
      apiErrorMessage.value = 'An error occurred while downloading the video.';
    }
    console.error(error);
  }
};
